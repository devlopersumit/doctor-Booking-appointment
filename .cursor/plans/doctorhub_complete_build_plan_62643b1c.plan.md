---
name: DoctorHub Complete Build Plan
overview: Complete implementation plan for DoctorHub - a full-stack MERN application for online doctor appointment booking with separate frontend and backend, JWT authentication, Context API for state management, and Tailwind CSS for styling.
todos:
  - id: setup-frontend
    content: Initialize React frontend with Tailwind CSS, install dependencies (axios, react-router-dom, fullcalendar, react-hook-form)
    status: pending
  - id: setup-backend
    content: Initialize Node.js backend with Express, install dependencies (mongoose, jwt, bcryptjs, nodemailer)
    status: pending
  - id: database-models
    content: "Create MongoDB models: User, Doctor, Appointment, Availability with proper schemas and relationships"
    status: pending
    dependencies:
      - setup-backend
  - id: auth-backend
    content: Implement authentication routes (register, login) with JWT, create auth middleware for protected routes
    status: pending
    dependencies:
      - database-models
  - id: api-routes
    content: "Create all API routes: doctors (list, detail, update), appointments (create, list, update), availability (CRUD)"
    status: pending
    dependencies:
      - auth-backend
  - id: frontend-context
    content: Set up React Context API for authentication state management, create AuthContext provider
    status: pending
    dependencies:
      - setup-frontend
  - id: auth-ui
    content: Build Login and Register pages with form validation, integrate with backend auth API
    status: pending
    dependencies:
      - frontend-context
      - auth-backend
  - id: doctor-listing
    content: Create doctor listing page with search/filter functionality, doctor cards, and detail view page
    status: pending
    dependencies:
      - auth-ui
  - id: appointment-booking
    content: Implement appointment booking form with FullCalendar integration, slot availability checking
    status: pending
    dependencies:
      - doctor-listing
      - api-routes
  - id: doctor-dashboard
    content: Build doctor dashboard with appointment management (view, accept, reschedule), availability management interface
    status: pending
    dependencies:
      - appointment-booking
  - id: styling-polish
    content: Apply Tailwind CSS styling throughout, ensure responsive design, add loading states and error handling
    status: pending
    dependencies:
      - doctor-dashboard
  - id: email-notifications
    content: Integrate nodemailer for sending appointment confirmation emails to patients and doctors
    status: pending
    dependencies:
      - appointment-booking
---
...
- `_id`, `name`, `email`, `password` (hashed), `role` (patient/doctor), `phone`, `createdAt`

### 2.2 Doctor Model (`backend/models/Doctor.js`)

- `_id`, `userId` (ref: User), `specialization`, `experience`, `qualifications`, `bio`, `rating`, `consultationFee`, `profileImage`

### 2.3 Appointment Model (`backend/models/Appointment.js`)

- `_id`, `patientId` (ref: User), `doctorId` (ref: Doctor), `date`, `time`, `status` (pending/confirmed/cancelled/completed), `notes`, `prescription`, `createdAt`

### 2.4 Availability Model (`backend/models/Availability.js`)

- `_id`, `doctorId` (ref: Doctor), `dayOfWeek` (0-6), `startTime`, `endTime`, `isAvailable`
